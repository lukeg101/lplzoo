...
- Inductive types are formed by prefixing a term with `in` and a type with `M` (or `μ`). Concretely, an inductive term looks like `in x:t` where `x` is the term and `t` is its type. Similarly, an inductive type looks like `μ(T)` where `T` is the type we insist is inductive. For example, peano nats have type `μ(1+X)` where `1` is zero, and `X` is a μ variable capturing the subterm of type `1+μ(1+X)` (also a nat). The example term zero captures this idea above. See the semantics for a description of what these constructions mean.
